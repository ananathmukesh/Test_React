...
const signup = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      json = httpstatus.invalidInputResponse({
    ...
    const checkuser_email = await db("users")
      .select("*")
      .where("email", email)
      .first();
    if (checkuser_email) {
      json = httpstatus.invalidResponse({
...
const otpverify = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      json = httpstatus.invalidResponse({
        message: "Invalid Otp No Pls Enter Valid Otp",
      });
      return res.end(json);
    }
...
const signin = async (req, res) => {
  ...
  try {
    ...
    if (otp_verfiy === "No") {
      json = httpstatus.invalidResponse({
        message:
          "Your Account is Not Verified, Pls verified your Account on otp",
      });
      return res.end(json);
    }
...
const logout = async (req, res) => {
  try {
    var json = httpstatus.successRespone({
      message: "Your Account has been Logouted",
      user: null,
    });
    return res.send(json);
  } catch (error) {
...
const getusers = async (req, res) => {
  ...
  try {
    ...
    const allUsers =
      await db.raw(`SELECT id,profile_image as img,CONCAT(firstname, ' ', lastname) as name,
  (SELECT TOP 1 time FROM message m WHERE m.sender_id=users.id ORDER BY message DESC) as time,
  (SELECT TOP 1 message FROM message m WHERE (m.sender_id=${id} and m.receiver_id=users.id) OR (m.sender_id=users.id and m.receiver_id=${id}) order by id desc) as msg,
...
const getGroupList = async (req, res) => {
  ...
  try {
    ...
    SELECT id, group_img as img, 'good morning everyone' as msg, group_name as name, '10:23' as time,groupmaster_id AS chatmasterid
    FROM groupchatmaster
    WHERE CHARINDEX('${id}', participant_id) > 0
  `);
  
  
    var json = httpstatus.successRespone({
      message: "Group Chat List",
      chatlist: allUsers,
      query: JSON.stringify(allUsers),
    });
    return res.send(json);
...
const Profileimage = async (req, res) => {
  try {
    ...
    try {
      await db("users").where({ id: 48 }).update({ profile_image: imagePath });
